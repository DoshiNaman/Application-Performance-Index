        ...
        <form method="POST" action="" class="p-2" enctype="multipart/form-data" class="table-responsive">
          <table class="table table-hover" id="table23">
            <thead>
              ...
              <th scope="col">Director Comment</th>
              ...
              <?php
                ...
                foreach($totalF as $name => $id) {
                   echo' <tr>
                        ...
                        if (mysqli_num_rows($result) > 0) {
                          while($row = mysqli_fetch_assoc($result)) {
                            ...
                            <td>
                                <div class="row">
                                    <div class="col-auto">
                                    <input type="text" class="form-control col-4" id="commentdata" name="commentdata'.$row["id"].'comment"  placeholder="Type comment.." value="'.$row["dcomment"].'" style="padding-y: 0.25rem;padding-x: 0.5rem;font-size: 0.875rem;border-radius: 0.25rem;">
                                    </div>
                                    <div class="col-auto">
                        ...
                        echo'
                        <td><button type="button" onclick="download(\''.$id.'\')" class="button-29 fs-6"><i class="fa-solid fa-download"></i></button></td>                    
                    </tr>';
                    $i=$i+1;
                }
              ?>
...
if(isset($_POST["comment"])){
  ...
  $abc= str_replace(' ','-',$abc);
  $cdata=$_POST[$abc];
  $sql = "UPDATE $tt SET dcomment='$cdata'  WHERE id='$cid'";
  // echo $sql;
